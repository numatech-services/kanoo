...
export default function ConnexionAdherentPage() {
  const [form, setForm] = useState({ email:"", accessCode:"" });
  const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  const router = useRouter();
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setLoading(true); setError("");
    const res = await fetch("/api/portail/auth", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({...form, type:"adherent"}) });
    const d = await res.json();
    if (d.success) router.push("/portail/adherent");
  ...
  return (
    <div className="max-w-sm mx-auto">
      <div className="text-center mb-8"><p className="text-3xl mb-2">🤝</p><h1 className="text-2xl font-bold text-ink">Espace adhérent</h1><p className="text-moss text-sm mt-1">Connectez-vous avec votre email et code d'accès</p></div>
      ...
      <p className="text-center text-xs text-moss mt-4">Code d'accès fourni par votre association</p>
    </div>
  );
}
